...
const statusIcon = (status: Status) => {
  switch (status) {
    ...
    case Status.Disconnected:
      return (
        <MdHighlightOff
          className="text-3xl text-yellow-500 dark:text-yellow-400"
          title="Desconectado"
        />
      );
    case Status.LoggedOut:
      return (
        <MdError
          className="animate-pulse text-3xl text-red-500 dark:text-red-400"
          title="Deslogado"
        />
      );
    case Status.Offline:
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between border-b bg-teal-50 p-2 dark:border-gray-800 dark:bg-gray-900">
        ...
        >
          <span
            className={`absolute left-0.5 top-1/2 h-5 w-5 -translate-y-1/2 rounded-full bg-white shadow transition-transform duration-200 dark:bg-gray-200 ${bot.Active !== false ? "translate-x-4" : ""} `}
          />
        </button>

        <button
          className={`flex items-center gap-1 rounded-full px-3 py-1 text-sm font-semibold transition-colors duration-200 ${
            bot.Paused
              ? "bg-yellow-400 text-yellow-900 hover:bg-yellow-500 dark:bg-yellow-500 dark:text-yellow-900 dark:hover:bg-yellow-400"
              : bot.Active === true
      ...
      {groupStats && onShowGroups && (
        <div className="flex items-center justify-between border-t p-2 dark:border-gray-700">
          ...
          <button
            className="rounded-full border-2 border-sky-400 bg-white/40 px-3 py-1 text-sm text-sky-700 backdrop-blur transition hover:bg-sky-50 dark:border-sky-500 dark:bg-gray-800/60 dark:text-sky-400 dark:hover:bg-gray-700"
      ...
      {typeof queueLength === "number" && (
        <div className="flex items-center justify-between border-t p-2 dark:border-gray-700">
          <div className="text-sm text-gray-600 dark:text-gray-300">
            {queueLength} mensage{queueLength === 1 ? "m" : "ns"} na fila
          </div>
          <button
            className="rounded-full border-2 border-yellow-400 bg-white/40 px-3 py-1 text-sm text-yellow-700 backdrop-blur transition hover:bg-yellow-50 dark:border-yellow-400 dark:bg-gray-800/60 dark:text-yellow-300 dark:hover:bg-yellow-900/40"
      ...
      {bot.sendingMessageInfo && bot.Status === Status.Sending && (
        <div className="flex flex-col gap-1 border-t p-2 dark:border-gray-700">
          ...
          <div className="flex items-center gap-2 rounded-sm bg-blue-50 p-1.5">
                  ...
                  width: `${Math.max(
                    ...
                    Math.min(
                      ...
                      ((bot.sendingMessageInfo.currentGroupIndex + 1) /
                        bot.sendingMessageInfo.totalGroups) *
                        100
                    )
                  )}%`,
                }}
            ...
            <span className="ml-2 text-xs text-blue-600 dark:text-gray-400">
              {bot.sendingMessageInfo.currentGroupIndex + 1} /{" "}
              {bot.sendingMessageInfo.totalGroups}
            </span>
          </div>
